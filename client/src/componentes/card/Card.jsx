...
const Card = ({ id, name, image, typesImg, types, imgDbPokemon, createdInDb, pokedex, attack }) => {
    ...
    return (
        ...
        <Link className={stylos.linkCard} to={`pokemons/${id}`}>
            ...
            <div className={stylos.mainContainer}>
                ...
                <p className={stylos.p}>Pokedex:{pokedex}</p>
                <p className={stylos.pattack}>Attack: {attack}</p>
                
                <div className={stylos.containerTypes}>

                    {createdInDb
                        ? types?.map((type) => (
                            <div className={stylos.p} key={`${type.name} ${id}`}>
                                {imgDbPokemon[0]?.url ? (
                        ...
                        : types?.map((type) => (
                            <div className={stylos.p} key={`${type} ${id}`}>
